...
conn.on('open', () => {
const remoteUserName = conn.metadata && conn.metadata.userName ? conn.metadata.userName : null;
if (this.isHost) {
    // 名前重複・無効チェック: 既に接続済みのユーザー名は拒否する
    if (!remoteUserName || this.users.includes(remoteUserName)) {
        try {
            conn.send({ type: 'system', sub: 'name-conflict' });
...
case 'heartbeat-ack':
    ...
    break;
case 'name-conflict':
    // ホストがユーザー名の重複を通知した場合は切断して理由をセット
    this._reset("ユーザー名が既に使用されています");
    break;
}
